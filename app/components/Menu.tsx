      ...
      <div className="container m-auto">
        ...
        <div className="sticky lg:relative top-[63px] lg:top-0 w-full overflow-x-auto">
          <ul className="relative flex flex-nowrap lg:flex-wrap justify-start lg:justify-center gap-2 lg:gap-4 h-full w-fit min-w-max lg:min-w-full py-2 z-30 mr-24 lg:mr-0">
            {['Ballagás', ...categories.slice(13)].map((category, index) => (
              <li
                key={index}
                className={`relative flex flex-row lg:flex-nowrap items-center p-1 gap-2 bg-[--okker] hover:bg-[--grey] rounded-md transition-all duration-200 cursor-pointer w-fit}`}
                onClick={() => handleCategoryClick(category)}
              >
                <div className="flex justify-center items-center w-[40px] h-[40px]">
                  <Image 
                    src={`ballagas-kalap.svg`} // Assuming this is the icon for 'Valentin nap'
                    alt={"Étel ikon"}
                    width={40}
                    height={40}
