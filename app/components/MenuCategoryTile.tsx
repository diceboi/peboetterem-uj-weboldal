    ...
    <div id={category} className={`flex flex-col w-full min-h-max pb-4 lg:pb-8 ${category === 'Valentin nap'? 'bg-gradient-to-br from-[#a1615c] to-[--grey]' : 'bg-[--grey]'} shadow-2xl rounded-md`}>
      ...
      {termekek.map((termek:any, index:any) => (
        <div key={index} className="flex flex-col lg:flex-row gap-2 justify-between items-start lg:items-end pt-4 pb-4 -mb-[1px] mx-4 lg:mx-16 border-b border-[--navy]">
            ...
            {alapadatokData && (
                <MenuButton title={`Kosárba ${termek.elsoelotag}`} icon={<TbShoppingCartPlus />} disabled={category === 'Valentin nap' ? false : isButtonDisabled} rendelesfelvetel={alapadatokData.rendelesfelvetel} termek={termek} tipus ={0}/>
              )}
              {termek.masodlagosar && (
                <MenuButton title={`Kosárba ${termek.masodikelotag}`} icon={<TbShoppingCartPlus />} disabled={category === 'Valentin nap' ? false : isButtonDisabled} rendelesfelvetel={alapadatokData.rendelesfelvetel} termek={termek} tipus={1}/>
              )}
            </div>
        </div>
      ))}
